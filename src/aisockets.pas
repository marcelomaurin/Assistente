...
{ Componentes de socket do AI Component Suite
  (https://github.com/marcelomaurin/CHATGPT - pacote openai_input).

  Substitui, neste projeto, o TLTCPComponent do pacote de terceiros lNet
  usado antes em ToolsFalar/ToolsOuvir/ToolsVer para conversar por TCP com o
  serviço externo de voz/visão.

  Removido em relação ao componente original apenas o registro de
  design-time (Register/ícone .lrs) e a dependência de LResources, já que
  aqui os componentes são criados em código (ver unit aiconn.pas), sem
  necessidade de aparecer na paleta da IDE. A lógica de conexão/envio/recepção
  é a mesma do componente original da suíte. }
